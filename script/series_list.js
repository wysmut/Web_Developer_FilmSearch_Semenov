...
async function getPopularMovies(page = 1) {
    try {
        ...
        // Добавлен параметр type=tv-series для получения сериалов
        const response = await fetch(
            `${BASE_URL}?page=${page}&limit=${requestLimit}&sortField=rating.kp&sortType=-1&type=tv-series`,
            {
...
async function searchMovies(query, filters = {}, page = 1) {
    try {
        // Добавлен параметр type=tv-series для поиска сериалов
        const params = new URLSearchParams({
            page,
            limit: requestLimit,
            type: 'tv-series', // Ключевое изменение
            ...filters
        });
        ...
        console.error('Ошибка при поиске сериалов:', error); // Изменен текст
        moviesGrid.innerHTML = '<div class="no-results">Ошибка загрузки данных. Пожалуйста, попробуйте позже.</div>';
        return [];
    }
}
...
function displayMovies(movies) {
    ...
    if (movies.length === 0) {
        moviesGrid.innerHTML = '<div class="no-results">Сериалы не найдены. Попробуйте изменить критерии поиска.</div>'; // Изменен текст
        return;
    }
    ...
    movies.forEach(movie => {
        ...
        // Добавлен значок "СЕРИАЛ"
        movieCard.innerHTML = `
            <div class="movie-poster-container">
                <img src="${movie.poster?.url || 'https://via.placeholder.com/300x450?text=No+Image'}"
                     alt="${movie.name}"
                     class="movie-poster">
                ${movie.isSeries ? '<div class="series-badge">СЕРИАЛ</div>' : ''}
            </div>
            <div class="movie-info-container">
                <h3 class="movie-title">${movie.name}</h3>
                <div class="movie-info">
                    <span class="movie-year">${movie.year || 'Н/Д'}</span>
        ...
        // Обработчик перехода на страницу сериала
...
// Основная функция загрузки сериалов
